...
const VOLUME_KEY = 'tvVolume:v1';

const VOLUME_STEP = 10; // percent per button press
...
try {
  ...
  if (Number.isFinite(savedVol) && savedVol >= 0 && savedVol <= 100) currentVolume = savedVol;
} catch {}
...
function getCurrentPlayer() {
  ...
  return players[youtubeId] || null;
}

function showVolumeOSD() {
  const bars = Math.round(currentVolume / 10);
  const bar = '█'.repeat(bars) + '░'.repeat(10 - bars);
  showOSD(`VOL  ${bar}  ${currentVolume}%`, 1500);
}
...
function bindImageMapActions() {
  mapAreas.forEach((area) => {
    area.addEventListener('click', (e) => {
      ...
      if (action === 'vol-up') volUp();
      if (action === 'vol-down') volDown();
...
document.addEventListener('keydown', (e) => {
  ...
  if (e.key === 'ArrowUp') { e.preventDefault(); volUp(); }
  if (e.key === 'ArrowDown') { e.preventDefault(); volDown(); }
